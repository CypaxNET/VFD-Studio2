...
begin
  ...
  begin
    ...
    begin
      ...
      if (FontName = '') or (FontSize = 0) then
      begin
...
begin
  ...
  begin
    ...
    if ((Trim(S) <> Trim(OldText)) or (Overlength > 0)) then
    begin    // repainting is only required if the text has changed or if it has overlength

      //while (Length(S)<43 ) do S:= S + ' ';   // string bis Displayende mit Leerstellen füllen
      if (FVariableInfo[I].FontName = '') or (FVariableInfo[I].FontSize = 0) then
      begin
        if (Overlength > 0) then
        begin  // the text is to long to be displayed
        ...
        begin
          while (FVariableInfo[I].PrevWidth > Length(S)) do
          begin
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if (FSMBios.HasProcessorInfo) and (I < Length(FSMBios.SystemSlotInfo)) then
        begin
  ...
  while (Pos('$CPUCORES$', S) <> 0) do
  begin
    I := Pos('$CPUCORES$', S);
    Delete(S, I, Length('$CPUCORES$'));
    if (FSMBios.HasProcessorInfo) then
    begin
...
begin
  ...
  begin
    ...
    if (Length(FCpuUsageData.UsageHistory) < TextWidth) then
    begin
    ...
    if (Length(FMemUsageData.UsageHistory) < TextWidth) then
    begin
      // if the array length is < than the text width of the display, then add a new array element
...
begin
  if (Length(FCpuUsageData.UsageHistory) > 0) then
  begin
    for I := High(FCpuUsageData.UsageHistory) downto 0 do
    begin
      CpuUsage := FCpuUsageData.UsageHistory[I];

      IntervalSize := 100 div FCpuUsageData.NumRows;

      for Row := 0 to (FCpuUsageData.NumRows - 1) do
      begin
...
var
  ...
  IntervalSize, LowerBound, UpperBound: Byte;
begin
  if (Length(FMemUsageData.UsageHistory) > 0) then
  begin
    for I := High(FMemUsageData.UsageHistory) downto 0 do
    begin
      MemUsage := FMemUsageData.UsageHistory[I];

      IntervalSize := 100 div FMemUsageData.NumRows;

      for Row := 0 to (FMemUsageData.NumRows - 1) do
      begin
        // specify boundaries of this row
        LowerBound := IntervalSize * Row;
        UpperBound := LowerBound + IntervalSize - 1;
