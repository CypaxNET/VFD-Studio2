/*!
 * @brief         Arduino driver for VGLEM1277 S1D15721 LCD
 * 
 ...
 *    accordingly to a VGLEM1277 display.
 *    Uses software-controlled flow control on the serial interface. Baud rate 115200.
 *
 *    Controller pins:
 *    Uses not connected analog input A0 to initialize the randomizer.
 *    
 *    Circuit:
 *    | Display pin (function) | Wemo D1 mini pin (function) |
 *    | ---------------------- | --------------------------- |
 *    | 1  (/RST )             | D2  (PD2)                   |
 *    | 2  (/CS)               | D8  (CS)                    |
 *    | 3  (A0)                | D1  (PD1)                   |
 ...
 *    | 5  (SI)                | D7  (MOSI)                  |
 *    | 6  (EN-BL)             | 5V                          |
 *    | 7  (PWM)               | D4  (PD4) or connect to 5V  |
 *    | 8  (VCC)               | 5V                          |
 *    | 9  (GND)               | GND                         |
 *    | 10 (GND)               | GND                         |
 * 
 * 
 * @copyright     Created August 2024 by Cypax, https://cypax.net
*/
...
//U8G2_S1D15721_240X64_F_4W_SW_SPI u8g2(U8G2_R2, /* clock=*/ 14, /* data=*/ 13, /* cs=*/ 15, /* dc=*/ 5, /* reset=*/ 4);
// A display constructor using hardware SPI:
U8G2_S1D15721_240X64_F_4W_HW_SPI u8g2(U8G2_R2, /* cs=*/ 15, /* dc=*/ 5, /* reset=*/ 4);
...
void setup()
{
  ...
  pinMode(PWM_PIN, OUTPUT); // used to control display backlight via PWM

  analogWrite(PWM_PIN, 255); // initially set to full brightness
      ...
      case 'T':
        // set display brightness or contrast
        {
          ...
          if ('\0' == *endptr[0])
          {
            ...
            // contrast is in percent, so we need to scale it to 0..255
            contrast = min(255, (int)ceil((float)contrast * 2.55f));
            analogWrite(PWM_PIN, contrast);
          } else {
